...
from src.models.layers import MultiConv, MultiConvRes2, ChannelAttention, SpatialAttention
...
class Unet(nn.Module):
    ...
    def __init__(self, inplanes, num_classes, width=32, norm_layer=None, deep_supervision=False, dropout=0,
        ...
        # 左侧卷积
        self.L1_conv1 = nn.Conv3d(self.in_channels, self.num * 1, kernel_size=3, stride=1, padding=2, dilation=2, bias=False)  # 4-32
        ...
        self.L1_res = nn.Conv3d(self.in_channels, self.num * 2, kernel_size=1, stride=1, padding=0, bias=False)  # 残差路径

        self.L2_conv1 = nn.Conv3d(self.num * 2, self.num * 2, kernel_size=3, stride=1, padding=2, dilation=2, bias=False)  # 64-64
        ...
        self.L2_res = nn.Conv3d(self.num * 2, self.num * 4, kernel_size=1, stride=1, padding=0, bias=False)  # 残差路径

        self.L3_conv1 = nn.Conv3d(self.num * 4, self.num * 4, kernel_size=3, stride=1, padding=2, dilation=2, bias=False)  # 128-128
        ...
        self.L3_res = nn.Conv3d(self.num * 4, self.num * 8, kernel_size=1, stride=1, padding=0, bias=False)  # 残差路径

        self.L4_conv1 = MultiConvRes2(self.num * 8, self.num * 8)  # 多尺度卷积残差2 256-256
        self.L4_bn1 = nn.BatchNorm3d(self.num * 8)
        self.L4_conv2 = MultiConvRes2(self.num * 8, self.num * 16)  # 多尺度卷积残差2 256-512
        ...
        # 最下层功能
        self.channel_attention = ChannelAttention(self.num * 16)  # 通道注意力
    ...
    def forward(self, x):
        ...
        res1 = self.L1_res(res1)  # 待残差
        x1 = torch.add(x1, res1)  # 残差
        ...
        res2 = self.L2_res(res2)  # 待残差
        x2 = torch.add(x2, res2)  # 残差
        ...
        res3 = self.L3_res(res3)  # 待残差
        x3 = torch.add(x3, res3)  # 残差
        ...
        # 最下层功能
        x4 = self.channel_attention(x4)  # 通道注意力
        # x4 = self.spatial_attention(x4)  # 空间注意力
