...
class Brats(Dataset):
    ...
    # __getitem__ 是一种特殊的方法，用于定义对象的索引操作。
    # 当你使用索引访问对象的属性或元素时（例如 obj[key]），Python 会尝试调用对象的 __getitem__ 方法来处理这个操作。
    # 当你使用 dataset[idx] 来获取数据集中的元素时，实际上就会调用 __getitem__ 方法，根据指定的索引值 idx 返回对应的数据。
    def __getitem__(self, idx):
        ...
        if self.normalisation == "minmax":
            patient_image = {key: irm_min_max_preprocess(patient_image[key]) for key in patient_image}  # 修正1%异常值
